...
valueOf r1 (IsZero expr1) k = valueOf r1 expr1 zeroCont
  where
    zeroCont val = applyCont k $ Bool $ val == Num 0
valueOf r1 (Let var expr1 body) k = valueOf r1 expr1 letCont
  where
    letCont val = valueOf (extend r1 var val) body k
valueOf r1 (IfExp expr1 expr2 expr3) k = valueOf r1 expr1 $ ifCont k
  where
    ifCont k val =
      if toBool val
        then valueOf r1 expr2 k
        else valueOf r1 expr3 k
valueOf r1 (Diff expr1 expr2) k = valueOf r1 expr1 diffCont1
  where
    diffCont1 val1 = valueOf r1 expr2 diffCont2
      where
        diffCont2 val2 = applyCont k $ Num $ toInt val1 - toInt val2
valueOf r1 (Application f x) k = valueOf r1 f operandCont
  where
    operandCont valFunc = valueOf r1 x operatorCont
      where
        operatorCont valArg =
